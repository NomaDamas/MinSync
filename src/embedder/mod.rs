...
pub fn create_embedder(config: &Config) -> Result<Box<dyn Embedder>> {
    let id = config.embedder.id.as_str();
    let batch_size = config.embedder.batch_size;
    if id.starts_with("openai:") {
        Ok(Box::new(openai::OpenAiEmbedder::from_env(id, batch_size)?))
        ...
        let embedder = tei::TeiEmbedder::new(id, &base_url, batch_size)
